...
def extract_topological_features(path: str) -> torch.Tensor:
    ...
    normalized = raw_features / (np.linalg.norm(raw_features) + 1e-8)
...
class SiameseSignatureModel(nn.Module):
    ...
    def __init__(self, pretrained_weights_path: str) -> None:
        ...
        self.projection_head = nn.Linear(fused_dim, 128)
        self.classifier = nn.Linear(256, 1)

    def _embed(self, img, topo):
        img_normalized = img.view(-1, 1, *Config.MODEL_INPUT_SIZE).float().div(255)

        cnn_emb = self.cnn_backbone(img_normalized)
        topo_emb = self.topo_projector(topo)

        fused = torch.cat([cnn_emb, topo_emb], dim=1)
        emb = self.projection_head(fused)
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, emb1, emb2, labels):
        ...
        loss_genuine = (1 - labels) * (1 - similarity).pow(2)
        loss_forged = labels * torch.clamp(similarity - self.margin, min=0).pow(2)
